...
If PYTHON_API_URL is set in the environment, forwards the request to the
Python model server running solution.py (via FastAPI / Flask).

Otherwise runs a deterministic mock so the UI works standalone.
"""
import os
import time
import requests
...
def predict_single(row: dict) -> tuple[dict, int]:
    """
    Returns (prediction_dict, duration_ms).
    prediction_dict: { 'User_ID': str, 'Purchased_Coverage_Bundle': int }
    """
    t0 = time.monotonic()
